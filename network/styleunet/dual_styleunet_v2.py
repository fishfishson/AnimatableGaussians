...
class DualStyleUNet_add_dwt(nn.Module):
    def __init__(self, inp_size, inp_ch, out_ch, out_size, style_dim, n_mlp, middle_size=8, c_dim=0,
                 channel_multiplier=2, blur_kernel=[1, 3, 3, 1], lr_mlp=0.01, debug=False):
        super().__init__()
        ...
        in_channel = self.channels[inp_size // 4]  # 64
        self.from_rgbs = nn.ModuleList()
        self.cond_convs = nn.ModuleList()
        self.comb_convs = nn.ModuleList()   # 64, 32, 16
        self.comb_convs.append(ConvLayer(in_channel * 2, in_channel, 3))

        self.conv_in = ConvLayer(inp_ch * 4, in_channel, 3, downsample=True)
        for i in range(int(math.log(inp_size, 2)) - 3, self.middle_log_size - 1, -1):  # 32, 16, 8
            out_channel = self.channels[2 ** i]  # (inp_size/2)->->(8*512)
            self.from_rgbs.append(FromRGB(in_channel, inp_ch, downsample=True, use_wt=True))  # //2
            # self.from_rgbs.append(FromRGB(in_channel, inp_ch, downsample=(i + 1)!=int(math.log(inp_size, 2)), use_wt=False))
            self.cond_convs.append(ConvBlock(in_channel, out_channel, blur_kernel))  # //2
            if i > self.middle_log_size:
                self.comb_convs.append(ConvLayer(out_channel * 2, out_channel, 3))
    ...
    def forward(
        ...
        # cond_list = self.img_unet(condition_img)
        if self.debug:
            print(f'[DEBUG] condition_img: {condition_img.shape}')
        cond_img = self.dwt(condition_img)
        if self.debug:
            print(f'[DEBUG] cond_img (after dwt): {cond_img.shape}')
        cond_out = self.conv_in(cond_img)  ### None
        if self.debug:
            print(f'[DEBUG] cond_out (after dwt): {cond_out.shape}')
        ...
        for cond_conv_id, (from_rgb, cond_conv) in enumerate(zip(self.from_rgbs, self.cond_convs)):
            ...
            cond_num += 1
            if self.debug:
                print(f'[DEBUG] cond_conv_{cond_conv_id:05d} cond_img: {cond_img.shape}, cond_out: {cond_out.shape}')
...
class DualStyleUNet_wo_dwt(nn.Module):
    def __init__(self, inp_size, inp_ch, out_ch, out_size, style_dim, n_mlp, middle_size=8, c_dim=0,
                 channel_multiplier=2, blur_kernel=[1, 3, 3, 1], lr_mlp=0.01):
        super().__init__()
        ...
        self.num_layers = (self.log_size - self.middle_log_size + 1) * 2
        for layer_idx in range(self.num_layers):
            res = (layer_idx + 8) // 2
            shape = [1, 1, 2 ** res, 2 ** res]
            self.noises.register_buffer(f"noise_{layer_idx}", torch.randn(*shape))

        for i in range(self.middle_log_size + 1, self.log_size + 2):  # 4, 5, 6, 7, 8, 9
            out_channel = self.channels[2 ** i]  # (16*512)->(32*512)->(64*512)->(128*256)->(256*128)->(512*64)
            ...
            self.to_rgbs1.append(ToRGB(in_channel=out_channel, style_dim=style_dim, out_channel=out_ch * 4, use_wt=False))
            ...
            self.to_rgbs2.append(ToRGB(in_channel = out_channel, style_dim = style_dim, out_channel = out_ch * 4, use_wt=False))

            in_channel = out_channel

        self.n_latent = self.log_size * 2 - (self.middle_log_size * 2 - 1) + 1 + 3
