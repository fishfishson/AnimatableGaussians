...
@torch.no_grad()
def calc_cano_weight_volume(data_dir, body_type: str = 'smplx', stage: int = 0):
    depth = 7
    ...
    if body_type == 'smplh':
        ...
        cano_pose = np.load(join(data_dir, 'output-output-smpl-3d', 'mesh-smplx/cano.npz'))
    ...
    if stage == 0:
        print('Stage 0: compute lbs gradients')
        cano_smpl_trimesh.export(join(data_dir, 'tmp/cano_smpl.ply'))
        if body_type == 'smplh':
        ...
        print('Stage 1: solved point interpolant and save cano weight volume')
        # solve(smpl_model.lbs_weights.shape[-1], ".\\bins\\PointInterpolant.exe", depth, tmp_dir)

    ### NOTE concatenate all grids
    fn_list = sorted(list(glob.glob(os.path.join(tmp_dir, 'grid_*.grd'))))
    assert len(fn_list) == smpl_model.lbs_weights.shape[-1]
...
if __name__ == '__main__':
    ...
    calc_cano_weight_volume(args.data_dir, args.body_type, args.stage)
